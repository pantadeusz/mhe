...
auto experiment_run = [](auto method_f, std::map<std::string, std::string> args) {
    ...
    auto ret_list = method_f(args); // wywolanie metody
    ...
    cout << "#" << args.at("-m") << "\t" << elapsed_seconds.count();
    if (args.count("-result"))
        cout << "\t" << best << endl;
    else
...
{ // przetwarzanie argumentów - stworzy nam mapę [nazwaargumentu,wartoscargumentu] - wygodnie obsłużymy sobie argumenty
    using namespace std;
...
{
    using namespace std;
    map<string, string> args = process_args(argc, argv); // przetwarzanie argumentów - stworzy nam mapę [nazwaargumentu,wartoscargumentu] - wygodnie obsłużymy sobie argumenty
    auto problem = make_shared<Problem>();

    if (args.count("-f"))
    {
        ifstream file(args["-f"]);
        file >> (*problem);
    }
    else
    {
