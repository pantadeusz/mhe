 ...
 * @arg fitness_f the function that calculates the fitness. It takes the specimen as an argument
 * @arg selection_f the selection function that gets the population (vector of the specimens ) and returns the index in the population
 * @arg crossover_f function performing crossover. It takes 2 arguments (the two specimens) and returns the two crossed over specimens
 * @arg mutation_f the mutation function that takes one specimen and returns also one specimen
 * @arg term_condition_f function that takes the population and the iteration index, and returns the true (continue) or false (finish)
 * 
...
auto genetic_algorithm = [](auto initial_population, auto fitness_f,
  ...
  while (term_condition_f(population,fit, iteration++)) {
    ...
    for (unsigned int i = 0; i < initial_population.size(); i++) {
      parents.push_back(population[selection_f(fit, iteration)]);
    }
    /// do the crossover
