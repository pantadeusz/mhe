...
function<double(work_point_t)> cost_function_factory(my_graph_t tsp_problem)
{
    return [=](work_point_t list_of_cities) -> double {
        double sum = 0;
        if (list_of_cities.size() == 0) throw std::invalid_argument("cities list must not be empty");
        ...
        if (sum == 0) throw std::invalid_argument("sum must not be 0");
        return sum;
    };
}
...
work_point_t generate_random_tsp_point(my_graph_t problem)
{
    ...
    if (result.size() == 0) throw std::invalid_argument("generated empty cities list");
    return result;
}
