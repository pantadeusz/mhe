...
auto arg = [](int argc, char** argv, std::string name,
...
#define OPTIMIZED ""
#endif
  std::cout << "# " << OPTIMIZED << "binary with date: " << __TIME__ << " "
            << __DATE__ << std::endl;
  std::cout << "-fname filename" << std::endl;
  std::cout << "-show_progress true/false" << std::endl;
}

std::pair<solution_t, std::chrono::duration<double>> experiment_full_revision(
    problem_t problem, bool show_progress) {
  using namespace std;
  ...
  /// goal function for solution - how good or bad it is. We minimize this
  /// function
  ...
  function<solution_t(solution_t)> next_solution = [](auto s) {
  ...
  solution_t best_solution = calculate_full_review<solution_t>(
      solution, goal, next_solution, term_condition, show_progress);
  auto calculation_end = chrono::steady_clock::now();
  /// show the result
  chrono::duration<double> calculation_duration =
...
std::pair<solution_t, std::chrono::duration<double>> experiment_random_probe(
    problem_t problem, int iterations, bool show_progress) {
  using namespace std;
  /// generate initial solution
...
std::pair<solution_t, std::chrono::duration<double>>
experiment_random_hillclimb(problem_t problem, int iterations,
                            bool show_progress) {
  using namespace std;
  /// generate initial solution
  solution_t solution = generate_random_solution(problem);
...
std::pair<solution_t, std::chrono::duration<double>> experiment_tabu(
    problem_t problem, int iterations, int max_tabu_size, bool show_progress) {
  using namespace std;
  /// generate initial solution
  solution_t solution = generate_random_solution(problem);
  ...
  /// generate next solution for the method
  function<vector<solution_t>(solution_t)> get_neighbors =
      [problem](auto s) -> vector<solution_t> {
    vector<solution_t> n;
  ...
  solution_t best_solution =
      calculate_tabu<solution_t>(solution, goal, get_neighbors, term_condition,
                                 max_tabu_size, show_progress);
  auto calculation_end = chrono::steady_clock::now();
  /// show the result
  chrono::duration<double> calculation_duration =
      calculation_end - calculation_start;
...
std::pair<solution_t, std::chrono::duration<double>>
experiment_simulated_annealing(problem_t problem, int iterations,
                               bool show_progress) {
  using namespace std;
  /// generate initial solution
  solution_t solution = generate_random_solution(problem);
  ...
  function<double(int)> temperature = [](int t) { return 1000 / t; };
  /// run the full review method
  auto calculation_start = chrono::steady_clock::now();
  solution_t best_solution = calculate_simulated_annealing<solution_t>(
      solution, goal, next_solution, temperature, term_condition,
      show_progress);
  auto calculation_end = chrono::steady_clock::now();
  /// show the result
  chrono::duration<double> calculation_duration =
...
std::pair<solution_t, std::chrono::duration<double>>
experiment_genetic_algorithm(problem_t problem, int iterations,
                             int population_size, double crossover_probability,
                             double mutation_probability, bool show_progress) {
  using namespace std;
  /// generate initial solution
  ...
  function<double(solution_t)> fitness_function = [problem](auto s) {
    ...
    double fit = 10000.0 +
                 1000.0 / (1 + tsp_problem_cost_function(problem, s)) -
                 error * 100;
  ...
  auto crossover = [=](solution_t a,
                       solution_t b) -> pair<solution_t, solution_t> {
    uniform_real_distribution<double> u(0.0, 1.0);
  ...
  auto crossover_pmx = [&](auto parent0, auto parent1) {
    if (parent0.size() != parent1.size())
      throw std::invalid_argument("both specimens should have the same size");
    uniform_int_distribution<int> distr(1, parent0.size() - 2);
    int a = distr(random_generator),
        b = distr(random_generator);  // punkty podzialu
    if (a > b) swap(a, b);
    ...
    // zamieniamy "głowę" i "ogon", a zostawiamy środek.
    for (int i = 0; i < parent0.size(); i = ((i + 1) == a) ? b : (i + 1)) {
      child0[i] = parent0[i];
      if (is_already_present(child0[i], child0)) {
        child0[i] = get_mapping(child0[i], parent0, parent1);
      }
      child1[i] = parent1[i];
      if (is_already_present(child1[i], child1)) {
  ...
  /// run the full review method
  auto calculation_start = chrono::steady_clock::now();
  solution_t best_solution = calculate_genetic_algorithm<solution_t>(
      init_population, fitness_function, selection_tournament,
      // crossover,
      crossover_pmx,
      mutation, term_condition, show_progress);
  auto calculation_end = chrono::steady_clock::now();
  /// show the result
  chrono::duration<double> calculation_duration =
      calculation_end - calculation_start;
...
int main(int argc, char** argv) {
  ...
  if (method == "full_revision") {
    ...
    auto [a, b] =
        experiment_random_hillclimb(problem, iterations, show_progress);
    best_solution = a;
    calculation_duration = b;
  } else if (method == "tabu") {
    auto [a, b] =
        experiment_tabu(problem, iterations, tabu_size, show_progress);
    best_solution = a;
    calculation_duration = b;
  } else if (method == "simulated_annealing") {
    auto [a, b] =
        experiment_simulated_annealing(problem, iterations, show_progress);
    best_solution = a;
    calculation_duration = b;

    // std::pair<solution_t,std::chrono::duration<double>>
    // experiment_genetic_algorithm(problem_t problem,

  } else if (method == "genetic_algorithm") {
    auto [a, b] = experiment_genetic_algorithm(
        problem, iterations, pop_size, crossover_p, mutation_p, show_progress);
    best_solution = a;
    calculation_duration = b;
  } else {
  ...
  cout << "# " << method
       << ": best_cost: " << tsp_problem_cost_function(problem, best_solution)
